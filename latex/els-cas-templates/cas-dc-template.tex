...
In addition to the alternative hydro management, the use of battery storage was evaluated for managing renewable generation and reducing curtailment. An endogenous capacity addition (subject to model optimization) was considered starting in 2028. Batteries were configured with a minimum state of charge of 15\% and a useful life of 15 years. In this approach, battery capital cost was removed as a limiting factor, allowing the model to allocate storage capacity purely on the basis of system operational needs, thereby revealing the upper bound of curtailment reduction achievable through storage deployment. This scenario is not intended to represent an economically viable configuration, but rather to characterize the maximum technical contribution of storage to renewable integration. The model allocated a maximum of 3100 MW of battery storage capacity between 2037 and 2043 (Figure \ref{fu} c)).
...
For 2040, the addition of extra DS capacity produces an increase in exports from the Centro region (hydro-dominated generation). Imports are diminished in the Norte region (also hydro-dominated), while increasing in Oriente and Sur, whose annual thermal generation is in turn reduced by approximately 1500 and 1000 GWh, respectively.
...
\begin{figure*}[tp]
	...
	\caption{\label{pow2} Hourly regional power supply (top row) and curtailed power (bottom row) for ALT-SD scenario and year 2050}
\end{figure*}
...
\begin{figure*}[tp]
	...
	\caption{\label{vert} Annual curtailed energy for ALT-SD scenario (a), after the implementation of the alternative hydro operation scheme (b) and the endogenous battery capacity addition (c)}
\end{figure*}
...
This motivated the analysis of two alternatives for mitigating this problem: the formulation of a hydroelectric operation scheme that, making use of Bolivia's high installed hydro capacity, shifts the current load-following-oriented paradigm to a balancing-oriented scheme; and the deployment of battery storage capacity. None of these alternatives were able to produce significant reductions in curtailment rates; there seems to be no margin for extra flexibility since thermal power plants are almost unused during solar production hours.
